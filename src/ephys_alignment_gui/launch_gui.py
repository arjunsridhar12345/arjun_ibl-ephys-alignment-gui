...
class MainWindow(QtWidgets.QMainWindow, ephys_gui.Setup):
    ...
    def _transform_to_ccf(self, image_physical_space_coordinates: npt.NDArray) -> None:
        ...
        if not smartspim_template_affine_transform:
            # try legacy way
            smartspim_template_affine_transform = tuple(self.input_path.glob('*/registration/ls_to_template_SyN_0GenericAffine.mat'))
            if not smartspim_template_affine_transform:
        ...
        if not smartspim_template_warp_transform:
            smartspim_template_warp_transform = tuple(self.input_path.glob('*/registration/ls_to_template_SyN_1InverseWarp.nii.gz'))
            if not smartspim_template_warp_transform:
