...
class EphysAlignment:
    ...
    def __init__(self, xyz_picks, chn_depths=None, track_prev=None,
        ...
        if np.any(track_prev):
            ...
            start_lims = 300
            self.track_init = np.array([-1 * 10, start_lims])
            self.feature_init = np.array([-1 * 10, start_lims])
    ...
    def _get_surface_intersection(self, traj, brain_atlas, surface='top', mode='raise'):
        ...
        idx_lim = np.sum(distance[dist_sort]  < np.max(brain_atlas.res_um))
    ...
    def scale_histology_regions(self, feature, track, region=None, region_label=None):
        ...
        region_label[:, 1] = np.flipud(region_label[:, 1])
    ...
    def get_channel_locations(self, feature, track, depths=None):
        ...
        channel_depths_track = self.feature2track(depths, feature, track) - self.xyz_track[0]
